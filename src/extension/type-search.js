(() => {
  ...
  const CONFIG = {
    idleMs: 2000,
    debounceMs: 120,
    rootId: "deadlock-dota-patch",
    pillId: "Deadlock2Dire-type-search",
    hitClass: "ddp-type-search-hit",
    currentClass: "ddp-type-search-current",
  };
  ...
  function searchTargets() {
    ...
    const selectors = [
      ".ddp-section-title",
      ".ddp-subsection-title",
      ".ddp-card-title",
      ".ddp-change-group-title",
      ".ddp-changes li",
    ];
  ...
  function searchTargetNode(node) {
    if (node.matches(".ddp-card-title")) return node.closest(".ddp-card");
    return node;
  }
  ...
  function renderPill() {
    ...
    pill.classList.toggle("ddp-type-search-empty", count === 0);
